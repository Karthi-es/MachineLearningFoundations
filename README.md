# MachineLearningFoundations
Posting my machine learning fundamentals learning notes here

Data preprocessing steps in basic ML procedure.
1. Import the dataset
2. Split independent and dependent variables into variables 'X' and 'Y
3. Handling missing values using SimpleImputer class
4. Encoding categorical varibales using OneHotEncoder
5. Split the variables into train set and test set
6. Performing feature scaling to the train dataset
